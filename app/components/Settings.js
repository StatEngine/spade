...
export default class Settings extends React.Component {
  ...
  static saveConfig(config) {
    ...
    fs.writeFileSync(configPath, formattedJson, 'utf8', (err) => {
      if (!err) {
  ...
  static intervalFromCron(cron) {
    const regex = /^0 \*\/(\d+)/g;
    const matches = regex.exec(cron || '0 */1 * ? * *');
  ...
  renderSources() {
    ...
    return sourcesKeys.map((sourceKey) => {
      ...
      // TODO: this can't assume 's3' in the future
      const destination = config.destinations[source.destination].s3

      return (
        <Card key={sourceKey} initiallyExpanded style={{ marginBottom: '16px' }}>
          <CardHeader
            actAsExpander
          ...
          <CardText expandable style={{ padding: '0 32px 16px' }}>
            ...
            <div style={{ margin: '20px 0' }}>
              <SelectField
                ...
                onChange={(e, index, value) => {
                  if (!(destination in config.destinations)) {
              ...
              <div>
                <TextField
                  ...
                  onChange={(e) => {
                    destination.folder = e.target.value;
                    this.saveSettings();
                  }}
                /><br />
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    destination.accessKeyId = e.target.value;
                    this.saveSettings();
                  }}
                /><br />
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    destination.secretAccessKey = e.target.value;
                    this.saveSettings();
                  }}
                /><br />
              </div>
